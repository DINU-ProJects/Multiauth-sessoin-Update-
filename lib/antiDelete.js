...
async function loadViewOnceData(remoteJid, messageId) {
    ...
    try {
        ...
        // FIX: Try local first, then GitHub
        if (meta.filePath && fs.existsSync(meta.filePath)) {
            return { buffer: fs.readFileSync(meta.filePath),...meta };
        } else if (meta.githubUrl) {
            console.log(`📥 Downloading from GitHub: ${meta.githubUrl}`);
            const buffer = await getFromGitHub(`viewonce/${getSafeJid(remoteJid)}/${messageId}.${meta.type === 'image'? 'jpg' : 'mp4'}`);
            if (buffer) {
                fs.writeFileSync(meta.filePath, buffer); // Cache locally
                return { buffer,...meta };
            }
        }
    } catch (e) {
...
async function handleMessageReaction(sock, reaction, from, botNumber) {
    ...
    // FIX: ViewOnce unlock on 👍 or any reaction
