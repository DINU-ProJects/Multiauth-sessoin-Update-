...
async function saveViewOnceMedia(remoteJid, messageId, buffer, messageType, mimetype, caption) {
    ...
    const ext = mimetype?.split('/')[1] || (messageType === 'image'? 'jpg' : 'mp4');
...
function loadViewOnceData(remoteJid, messageId) {
    ...
    try {
        ...
        const ext = meta.mimetype?.split('/')[1] || (meta.type === 'image'? 'jpg' : 'mp4');
        const filePath = path.join(dirPath, `${messageId}.${ext}`);

        if (fs.existsSync(filePath)) {
            return { buffer: fs.readFileSync(filePath),...meta };
        }
    } catch (e) {}
    return null;
}
...
async function handleMessageReaction(sock, reaction, from, botNumber) {
    ...
    if (viewOnceData) {
        ...
        const sentBy = loadMessageData(remoteJid, messageId)?.sender || 'Unknown';
...
async function handleMessageRevocation(sock, revocation, from, botNumber) {
    ...
    try {
        ...
        else if (messageType === 'imageMessage' || originalMessage.isViewOnce && messageType === 'imageMessage') {
            try {
        ...
        else if (messageType === 'videoMessage' || originalMessage.isViewOnce && messageType === 'videoMessage') {
            try {
